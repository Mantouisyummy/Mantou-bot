...
async def add_money(bot:Bot, interaction:Interaction, id:int, money:int):
    ...
    try:
        ...
        embed = Embed(title=f"✅ | 成功",description=f"已給予 {user.mention} `{money}$`\n他目前的餘額 `{new_money}$`",colour=Colour.green())
...
async def remove_money(bot:Bot, interaction:Interaction, id:int, money:int):
    ...
    try:
        ...
        with open(f"./database/users/{id}.json","w+",encoding='utf-8') as f:
                json.dump(data,f)
        user = bot.get_user(id)
        embed = Embed(title=f"✅ | 成功",description=f"已減少 {user.mention} `{money}$`\n他目前的餘額:`{new_money}$`",colour=Colour.green())
        await interaction.response.send_message(embed=embed)
    except KeyError:
        await initialization(bot,interaction,id,object="nokey")
        await interaction.response.defer()
        ...
        if choice == "upload":
            ...
            if data["bag"] == {}: 
                ...
                try:
                    ...
                    with open(f"./database/users/{id}.json","w+",encoding='utf-8') as f:
                        json.dump(data,f)
                    embed = Embed(title=f"✅ | 你出售了 {item} x{limit}個! 獲得了這是放錢的地方$",colour=Colour.green())
                    await interaction.response.send_message(embed=embed)
                except KeyError:
